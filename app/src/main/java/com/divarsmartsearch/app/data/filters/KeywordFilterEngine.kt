...
object KeywordFilterEngine {
    ...
    /**
     * Public so other checks in this package can normalize text the same
     * way before matching — otherwise the same
     * Yeh/Kaf/ZWNJ variants that this filter accounts for would silently
     * break their `contains(...)` checks on real Divar text.
     *
     * Bug fix: multi-word filters (e.g. "بدون واسطه", "کد ملک", "مشاور
     * املاک") used to fail silently whenever the real scraped text split
     * those same two words across a line break or extra spacing — very
     * common, since card/description text comes from `innerText` and is
     * naturally multi-line, while the person types their filter with a
     * single plain space. A run of ANY whitespace (space, tab, newline) is
     * now collapsed to exactly one space on both sides before matching, so
     * a phrase filter matches regardless of how the real text happens to
     * be broken up.
     */
    ...
    fun normalize(text: String): String {
        ...
        return collapsed.replace(WHITESPACE_RUN_REGEX, " ").trim()
    }
