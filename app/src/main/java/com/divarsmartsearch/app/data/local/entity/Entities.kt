...
data class ListingEntity(
    ...
    // Bug fix: if the one-time detail-page fetch above ever failed or
    // timed out (a transient network hiccup, Divar briefly rate-limiting,
    // etc.), `hasDetailDescription` stayed false forever and — since a
    // repeated list-preview scrape of the same still-visible card was
    // deliberately never treated as a "change" (see the bug-fix note in
    // ListingIngestionService.ingest()) — the listing was never retried
    // again. That meant the keyword-exclude filter could only ever check
    // the short, sometimes-incomplete card-preview text for that listing,
    // permanently, even though the real detail page might contain a
    // forbidden word the preview didn't. Retrying up to
    // MAX_DETAIL_FETCH_ATTEMPTS times before giving up fixes that while
    // still avoiding hammering a listing whose detail page is genuinely
    // gone (404, deleted, etc.) forever.
    var detailFetchAttempts: Int = 0,
 ...
 * where [isEnabled] is true, and a match on ANY enabled row rejects the
 * listing — that is now the ONLY thing that can reject a listing on
 * content grounds (see [com.divarsmartsearch.app.data.filters.FilterPipeline]).
 *
 * [filterType] is legacy: an older version of the app had a separate
 * "owner_signal" type whose match overrode exclude filters and skipped an
 * automatic AI/heuristic agency-probability check. Both of those (the
 * override and the AI check itself) were removed per explicit user
 * request, and the UI no longer offers "owner_signal" as an option — every
 * filter added from the UI is "exclude". The column is kept only so old
 * rows don't need a destructive migration; it no longer changes behavior.
 *
...
data class KeywordFilterEntity(
    ...
    val label: String,
    val keyword: String,
    val category: String = "custom", // "real_estate" | "consultant" | "office" | "key" | "custom"
    val filterType: String = "exclude", // legacy column, always "exclude" now — see KDoc above
