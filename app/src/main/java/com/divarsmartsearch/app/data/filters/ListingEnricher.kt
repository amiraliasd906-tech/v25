 ...
 * stored listings — something the pure, single-ad check in
 * KeywordFilterEngine can't do on its own:
 *
 *  - phoneRepeatCount: how many other ads (any search, any time) share one
 *    of this ad's phone numbers.
...
class ListingEnricher @Inject constructor(
     ...
     *
     * Bug fix, per explicit user request: this used to also give +1 for
     * `ownerProbability >= 0.8`. That field is fixed at exactly 1.0 for
     * EVERY visible listing now (the old graded agency-probability stage
     * was removed from FilterPipeline entirely), so that branch always
     * fired and gave every single listing the same free +1 — not a real
     * signal, just noise. Removed; only genuine cross-listing signals
     * (price vs. area average, duplicate/republish, phone repeat count)
     * affect the rating now.
     */
