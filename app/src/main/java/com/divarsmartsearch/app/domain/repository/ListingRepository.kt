...
interface ListingRepository {
     ...
     *
     * @param withDetailRefetch If true, listings still missing their real
     * detail-page description (see [com.divarsmartsearch.app.data.local.entity.ListingEntity.hasDetailDescription])
     * get ONE bounded attempt to fetch it before filters are checked, so a
     * keyword that only appears in the ad's full body text (not the short
     * results-card preview) can actually be caught. This costs real time
     * (a handful of headless page loads), so it defaults to false — plain
     * refreshes (e.g. pull-to-refresh on Results) stay fast and only
     * re-check whatever text is already stored; pass true only right after
     * the person adds/enables a keyword filter, where it's worth the wait.
     *
     ...
     */
    suspend fun reapplyFilters(withDetailRefetch: Boolean = false): AppResult<Int>
}
