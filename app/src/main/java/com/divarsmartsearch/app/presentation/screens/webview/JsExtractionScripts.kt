        ...
        (function () {
          try {
            ...
            function extractDetailPage() {
              ...
              // Per explicit user request: capture the WHOLE visible ad —
              // address, spec table (rooms/build year/area), publish time
              // ("۵ روز پیش"), and the description section — not just the
              // text under the "توضیحات" heading. Starting from the
              // title's own position in the page's visible text (skipping
              // only the few small header icons above it) down to the
              // "اطلاعات تماس" contact button, which reliably marks the
              // end of the ad's own content on every listing.
              var bodyText = document.body.innerText || '';
              var titleIdx = title ? bodyText.indexOf(title) : -1;
              var contentStart = titleIdx !== -1 ? titleIdx : 0;
              var afterTitle = bodyText.substring(contentStart);
              var endMarkers = ['اطلاعات تماس', 'گزارش تخلف', 'آگهی\u200cهای مشابه', 'ثبت آگهی مشابه'];
              var endIdx = afterTitle.length;
              for (var mi = 0; mi < endMarkers.length; mi++) {
                var markerIdx = afterTitle.indexOf(endMarkers[mi]);
                if (markerIdx !== -1 && markerIdx < endIdx) endIdx = markerIdx;
              }
              var extracted = afterTitle.substring(0, endIdx).trim().substring(0, 6000);
              var description = extracted.length > 20 ? extracted : null;
              // Fallback for the rare page where none of this worked out
              // (title not found verbatim in body text, etc.): combine
              // every real <p> on the page instead of giving up entirely.
              if (!description) {
                ...
                for (var pi = 0; pi < paragraphs.length; pi++) {
                  if (!seenText[paragraphs[pi]]) {
                    seenText[paragraphs[pi]] = true;
                    uniqueParagraphs.push(paragraphs[pi]);
                  }
                }
                var combined = uniqueParagraphs.join('\n').substring(0, 6000);
                if (combined.length > 20) description = combined;
              }
